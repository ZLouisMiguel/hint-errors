...
/**
 * @fileoverview Browser demo engine for the hint-errors GitHub Pages
 * playground. The `hints` array and `getHint()` function below are copied
 * verbatim from src/hints.js so the demo always reflects the real matching
 * logic shipped in the npm package. `renderFormatted()` mirrors the field
 * layout of src/formatter.js, but renders to DOM/CSS instead of ANSI codes
 * since there's no terminal here.
 *
 * Deliberately NOT reproduced: parseError()'s stack-frame filtering. That
 * logic depends on a real Node.js Error.stack and process.cwd(), neither of
 * which map onto a browser demo in a way that would be honest to show. The
 * "location" field below always falls back to "unknown location", exactly
 * like the real formatError() does when parsed.file is null.
 */

// ─── Copied verbatim from src/hints.js ──────────────────────────
const hints = [
  // ─── TypeErrors ────────────────────────────────────────────────
  {
 ...
 * Identical logic to getHint() in src/hints.js.
 *
 * @param {{type: string, message: string}} parsed
 * @returns {string}
 */
...
// ─── Browser rendering (mirrors src/formatter.js field layout) ──

/**
 * Escapes HTML-unsafe characters so error messages typed by visitors can't
 * inject markup into the demo terminal.
 * @param {string} str
 * @returns {string}
 */
...
/**
 * Renders the same four fields formatError() writes to stdout — error,
 * message, location, hint — as a DOM node styled like the CLI's colorized
 * output. "location" always reads "unknown location" because this demo
 * doesn't have a real Node.js stack trace to parse (see file header).
 *
 * @param {{type: string, message: string}} parsed
 * @param {string} hint
 * @returns {HTMLElement}
 */
function renderFormatted(parsed, hint) {
  const block = document.createElement("div");
  block.className = "term-block";

  const fields = [
    { key: "error", value: parsed.type, cls: "f-red" },
    { key: "message", value: parsed.message, cls: "f-bold" },
    { key: "location", value: "unknown location", cls: "f-yellow" },
    { key: "hint", value: hint, cls: "f-cyan" },
  ];

  for (const { key, value, cls } of fields) {
    const row = document.createElement("div");
    row.className = "term-row";
    row.innerHTML = `<span class="term-key">${key}</span><span class="term-value ${cls}">${escapeHtml(
      value,
    ).replace(/\n/g, "<br>")}</span>`;
...
const PRESETS = [
  {
    label: "TypeError — undefined property read",
    type: "TypeError",
    message: "Cannot read properties of undefined (reading 'name')",
  },
  {
    label: "TypeError — null property read",
    type: "TypeError",
  ...
  {
    label: "ReferenceError — window undefined",
    type: "ReferenceError",
    message: "window is not defined",
  },
  {
    label: "ReferenceError — undeclared variable",
    type: "ReferenceError",
  ...
  {
    label: "RangeError — max call stack",
    type: "RangeError",
  ...
  {
    label: "Error — unmatched (generic fallback)",
    type: "Error",
...
function init() {
  ...
  const typeInput = document.getElementById("errorType");
  const messageInput = document.getElementById("errorMessage");
  ...
  function run() {
    const parsed = {
      type: typeInput.value.trim() || "Error",
      message: messageInput.value.trim() || "An unknown error occurred",
    };
    const hint = getHint(parsed);
    output.innerHTML = "";
    output.appendChild(renderFormatted(parsed, hint));
  }

  // Seed with the first preset on load.
